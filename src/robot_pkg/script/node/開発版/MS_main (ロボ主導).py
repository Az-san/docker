...
class StartTrial(
    ...
    def execute(
        ...
        self._lib["record"].saveInfo() # 実験情報のテキストファイル保存
        ...
        rospy.set_param(rospy.get_name() + "/unexp_points_meter", []) #未探索進行可能方向情報
...
class CheckIntersection(
    ...
    def execute(
        ...
        # 交差点を見つけた場合
            ...
            #地図の保存とGUIの変更
            self._lib["record"].saveMap()
            self._lib["if"].changeGUI(position_meter[:2])
            rospy.sleep(1.0)
            inf_rad_sorted = self._lib["scan"].getLaserInfRad()
            position_meter = self._lib["map"].getRobotPointMeter()
            rospy.set_param(rospy.get_name() + "/inf_rad_sorted", inf_rad_sorted)
            ...
            print("irs={},nir={},pm2={}".format(inf_rad_sorted,nearest_inf_rad_to_present_rad,position_meter[2]))
...
class MakeHumanDecideDirection(
    ...
    def execute(
        ...
        # ゲームパッドで被験者が選択した方向を出力する(タイムアウトの場合ロボット主導になる)
        decided_direction = []
        while True:
            human_direction = self._lib["if"].waitGamepad(timeout = 15)                                                     # [上、下、右、左]

            if human_direction == None:
                print("input timeout ...")
                decided_direction = [0,0,0,0]
                break
            ...
            if direction_candidates_inf_rad_sorted_idx[[i for i in range(len(human_direction)) if human_direction[i]==1][0]] == -1:
                ...
                direction_candidates_UDRL=[direction_candidates_UDRL[i] if human_direction[i]==0 else -1 for i in range(len(direction_candidates_UDRL))]
                decided_direction = self._lib["if"].changeGamepadInputUDRLtoNSEW(human_direction, position_meter[2])  # [北、南、東、西]
                break

        rospy.set_param(rospy.get_name() + "/human_decided_direction", decided_direction) # [北、南、東、西]
        rospy.set_param(rospy.get_name() + "/direction_candidates_UDRL", direction_candidates_UDRL) #人間の選んだ選択肢を消した
...
class MakeRobotDecideDirection(
    ...
    def execute(
        ...
        decided_direction = []
        # random時であれば、人間選択方向と更新方向以外で進行可能な中からランダムに選ぶ(3差路ならただ一つに決まる)
        if rospy.get_param("/random", "") == True:
            direction_candidates_UDRL[1] = -1 #後進方向は除外
            decided_direction = [0 if direction_candidates_UDRL[i]==-1 else 1 for i in range(len(direction_candidates_UDRL))]
            ...
            decided_direction = self._lib["if"].changeGamepadInputUDRLtoNSEW(decided_direction, position_meter[2])
            
        # not random時であれば、人間が選んだ方向以外で右手法or左手法でロボットが選択する
        elif rospy.get_param("/random", "") == False:
            decided_direction = [0 if direction_candidates_UDRL[i]==-1 else 1 for i in range(len(direction_candidates_UDRL))]
            if sum(decided_direction) > 1:         #候補が複数方向のとき右手法or左手法
...
class DecideDirection(
    ...
    def execute(
        ...
        decided_direction = []
        if human_decided_direction == [0,0,0,0]: #人間側タイムアウトの場合
            pilot = "robot"
        else:
            pilot = random.choice(("human", "human"))#, "human", "human"))
