...
## @modified for python3, PyQt5 by Takumi FUJI
## @brief ライブラリクラス
#==================================================
...
class GUI(QtWidgets.QWidget):
    ...
    #==================================================
    def mapimg(self):
        self.mapimg = cv2.imread(roslib.packages.get_pkg_dir("robot_pkg") + "/io" + "/titlex2.jpg")
        color_lower = np.array([204, 204, 204, 255])                 # 抽出する色の下限(BGR形式)
        color_upper = np.array([204, 204, 204, 255])                 # 抽出する色の上限(BGR形式)
        img_mask = cv2.inRange(self.mapimg, color_lower, color_upper)    # 範囲からマスク画像を作成
        img_bool = cv2.bitwise_not(self.mapimg, self.mapimg, mask=img_mask)      # 元画像とマスク画像の演算(背景を白くする)
