...
class StartTrial(
    ...
    def execute(
        ...
        rospy.set_param(rospy.get_name() + "/inf_rad_sorted", []) #進行可能方向
        rospy.set_param(rospy.get_name() + "/position_meter", []) #現在位置
        ...
        rospy.set_param(rospy.get_name() + "/errp", 0) #Errpが期待されるかどうか
...
class MakeDirectionCandidates(
    ...
    def execute(
        ...
        print("direction_candidates_UDRL = {}".format(direction_candidates_inf_rad_sorted_idx))
...
class MakeHumanDecideDirection(
    ...
    def __init__(
        ...
        self._lib = lib
        self.Skt = socket_server.BMISocket() #ソケット通信作成
    ...
    def execute(
        ...
        # ゲームパッドで被験者が選択した方向を出力する(左手法)
        decided_direction = []
        while True:
            human_direction = self.Skt.readConv()                                                     # [上、下、右、左]
...
class MakeRobotDecideDirection(
    ...
    def execute(
        ...
        if rospy.get_param("/random", "") == True:
            ...
            rand_direction = random.choice(direction_candidates_inf_rad_sorted_idx_trim)
            decided_direction = [1 if direction_candidates_inf_rad_sorted_idx[i]==rand_direction else 0 for i in range(len(direction_candidates_inf_rad_sorted_idx))]
        # not random時であれば、人間が選ばなかった後進以外の方向を選ぶ(4叉路ならどちらかランダム)
