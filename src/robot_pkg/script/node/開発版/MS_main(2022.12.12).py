...
class MakeHumanDecideDirection(
    ...
    def execute(
        ...
        decided_direction = []
        human_direction = self._lib["if"].changeGamepadInputUDRLtoNSEW([0,0,1,0], position_meter[2])  # [北、南、東、西]
        if direction_candidates_inf_rad_sorted_idx[[i for i in range(len(human_direction)) if human_direction[i]==1][0]] == -1:
            human_direction = self._lib["if"].changeGamepadInputUDRLtoNSEW([1,0,0,0], position_meter[2])  # [北、南、東、西]
            if direction_candidates_inf_rad_sorted_idx[[i for i in range(len(human_direction)) if human_direction[i]==1][0]] == -1:
                human_direction = self._lib["if"].changeGamepadInputUDRLtoNSEW([0,0,0,1], position_meter[2])  # [北、南、東、西]
                if direction_candidates_inf_rad_sorted_idx[[i for i in range(len(human_direction)) if human_direction[i]==1][0]] == -1:
                    human_direction = self._lib["if"].changeGamepadInputUDRLtoNSEW([0,1,0,0], position_meter[2])  # [北、南、東、西]
