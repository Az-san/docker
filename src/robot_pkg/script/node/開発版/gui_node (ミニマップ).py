...
class GUI(QtWidgets.QWidget):
    ...
    def paintEvent(self, e):
        ...
        if self.gui_state != "move" and self.gui_state != "wait" and self.gui_state != "select" and self.gui_state != "error":
            if self.gui_state == "start":
                ...
                position_meter_str = self.gui_state.split(",")
                position_meter = [float(s) for s in position_meter_str]
            self.load_mapimg(position_meter)
        if self.gui_state != "start":
    ...
    ## @return
    #==================================================
    def load_mapimg(self, position_meter = [0,0]):
        try:
            ...
            pos_pix = (- int(position_meter[1] / 0.05) +14, self.map_img.shape[0] -25 - int(position_meter[0] / 0.05))
            cv2.circle(self.map_img, pos_pix, 6, (255,0,0), thickness = -1)
