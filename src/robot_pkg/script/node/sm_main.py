...
class Start(
    ...
    def execute(
        ...
        if rospy.get_param("/exp_type", "") == "be":
            if rospy.get_param("/data_file", "") == "exp":
                exp_cnt = self._lib["record"].getExpCnt()
                self._lib["record"].initSaveDataPath(exp_cnt)

                # 迷路を変更する
                self._lib["nav"].changeMazeModel(exp_cnt)

                # マップを変更する
                """
                sub = QTABLE_DATA_LIST[exp_cnt][1]
                file = ""
                for sub_name in os.listdir(roslib.packages.get_pkg_dir("robot_pkg") + "/doc/exp_data"):
                    if QTABLE_DATA_LIST[exp_cnt][1] in sub_name:
                        file = sub_name
                num = QTABLE_DATA_LIST[exp_cnt][2]
                maze = "7" if QTABLE_DATA_LIST[exp_cnt][3] == "1" else "8"
                goal = QTABLE_DATA_LIST[exp_cnt][4]
                path = roslib.packages.get_pkg_dir("robot_pkg") + "/doc/exp_data/{}/nakamura/{}{}{}{}/map.yaml".format(file, sub, num, maze, goal)
                self._lib["map"].changeMap(path)
                """

                # 迷路の種類によってGOALを変更する
                if QTABLE_DATA_LIST[exp_cnt][3] == "1":
                    if QTABLE_DATA_LIST[exp_cnt][4] == "A":
                        self._lib["nav"].setGoalPointMeter(GOAL_POINT_METER_LIST[0][0])
                    elif QTABLE_DATA_LIST[exp_cnt][4] == "B":
                        self._lib["nav"].setGoalPointMeter(GOAL_POINT_METER_LIST[0][1])
                    else:
                        print("GOAL_POINT_METER error !!")
                elif QTABLE_DATA_LIST[exp_cnt][3] == "2":
                    if QTABLE_DATA_LIST[exp_cnt][4] == "A":
                        self._lib["nav"].setGoalPointMeter(GOAL_POINT_METER_LIST[1][0])
                    elif QTABLE_DATA_LIST[exp_cnt][4] == "B":
                        self._lib["nav"].setGoalPointMeter(GOAL_POINT_METER_LIST[1][1])
                    else:
                        print("GOAL_POINT_METER error !!")
                else:
                    print("Xtable error !!")

                print("GOAL_POINT_METER = {}".format(self._lib["nav"].getGoalPointMeter()))
...
class StartTrial(
    ...
    def execute(
        ...
        if exp_cnt == len(QTABLE_DATA_LIST):

            return "except"

        if self._lib["record"].getTrialCnt() >= TRIAL_ITERATION_NUM:
            if rospy.get_param("/data_file", "") == "exp":
                #self._lib["nav"].resetTrial()
                self._lib["record"].incExpCnt()

                return "restart"

            else:
                self._lib["nav"].exitTrial()
        ...
        # be時であれば、QとXを初期化せず、記録したQとXを使用する
        if rospy.get_param("/exp_type", "") == "be":
            
            print("### Behavioral experiment ###")
            if rospy.get_param("/data_file", "") == "":
                rospy.set_param(rospy.get_name() + "/q_table", [])
                rospy.set_param(rospy.get_name() + "/x_table", [])
            elif rospy.get_param("/data_file", "") == "exp":
                q_table = QTABLE_DATA_LIST[exp_cnt][6]

                # 迷路の種類によってXtableを変更する
                if QTABLE_DATA_LIST[exp_cnt][3] == "1":
...
class CheckIntersection(
    ...
    def execute(
        ...
        # 交差点を見つけた場合
            ...
            rospy.loginfo("I found intersection.")
...
class GoAhead(
    ...
    def execute(
        ...
        if self._lib["scan"].getAheadDistance() < 1.0:
            ...
            nearest_inf_rad_to_present_rad = inf_rad_sorted[0]
            for i in range(len(inf_rad_sorted)):
                if math.cos(inf_rad_sorted[i]) >= math.cos(nearest_inf_rad_to_present_rad):
                    nearest_inf_rad_to_present_rad = inf_rad_sorted[i]
            # ロボットを回転させる（位置調整のため）
            self._lib["nav"].sendRotationTwist(nearest_inf_rad_to_present_rad, 0.0)
            """
            rospy.sleep(1.0)
...
class MakeRobotDecideDirection(
    ...
    def execute(
        ...
        # noy random時であれば、進行可能な方向から、q_tableが最大の進行方向を選択する
            ...
            for i in range(len(q_trim)):
                if max_q_trim < q_trim[i]:
                    max_q_trim = q_trim[i]
                    max_q_trim_idx = i
                elif max_q_trim == q_trim[i]:
                    rand = random.randrange(2)
                    if rand == 0:
                        max_q_trim_idx = i
            max_q_direction = direction_candidates_inf_rad_sorted_idx_trim[max_q_trim_idx]
            """
...
class DecideDirection(
    ...
    def execute(
        ...
        decided_direction = []
        # be時であれば、ロボットの決定した進行方向のみに移動する
        if rospy.get_param("/exp_type", "") == "be":
            print("< be > chose robot's decided direction !")
            pilot = "robot"
            decided_direction = robot_decided_direction
        # ee時であれば、人が決定した進行方向、もしくはロボットが決定した進行方向へランダムに移動する
        elif rospy.get_param("/exp_type", "") == "ee":
            pilot = random.choice(("robot", "human"))
            if pilot == "robot":
...
class CheckErrP(
    ...
    def execute(
        ...
        # 後退方向が選択された場合、ロボットが少し後退する(ErrP誘発のため)
...
class GoToCorrectDirection(
    ...
    def execute(
        ...
        # be時かつexp時にロボットが未探索地域に移動してしまった場合、ストップウォッチを終了してトライアルをリセットする
        if rospy.get_param("/exp_type", "") == "be":
            if rospy.get_param("/data_file", "") == "exp":
                unexp_points_meter = rospy.get_param(rospy.get_name() + "/unexp_points_meter", [])
                if len(unexp_points_meter) != 0:
                    for i in range(len(unexp_points_meter)):
                        print("unexp_points_meter[i] = {}".format(unexp_points_meter[i]))
                        print("position_meter        = {}".format(position_meter))
                        distance = math.sqrt((unexp_points_meter[i][0] - position_meter[1])**2 + (unexp_points_meter[i][1] - position_meter[0])**2)
                        print("distance              = {}".format(distance))
                        if distance < 1.0:
                            print("im at unexp ...")
                            self._lib["record"].recordTime(state="stop")
                            self._lib["record"].recordGoal(False)
                            self._lib["record"].recordStatus("go to unexp")
                            self._lib["record"].incTrialCnt()
                            self._lib["nav"].resetTrial()

                            return "reset"

        # be時かつゴール近辺1.0m到着であれば、ストップウォッチを終了してトライアルをリセットする
        if rospy.get_param("/exp_type", "") == "be":
            distance = math.sqrt((self._lib["nav"].getGoalPointMeter()[0] - position_meter[0])**2 + (self._lib["nav"].getGoalPointMeter()[1] - position_meter[1])**2)
            if distance < 1.0:
                print("im at goal !")
                self._lib["record"].recordTime(state="stop")
                self._lib["record"].recordGoal(True)
                self._lib["record"].recordStatus("goal")
                self._lib["record"].incGoalCnt()
                self._lib["record"].incTrialCnt()

                """
                if self._lib["record"].getGoalCnt() >= GOAL_ITERATION_NUM:
                    if rospy.get_param("/data_file", "") == "exp":
                        self._lib["nav"].resetTrial()
                        self._lib["record"].incExpCnt()
        ...
        for i in range(len(inf_rad_sorted)):
            if math.cos(inf_rad_sorted[i] - position_meter[2]) >= math.cos(nearest_inf_rad_to_present_rad - position_meter[2]):
                nearest_inf_rad_to_present_rad = inf_rad_sorted[i]
        # ロボットの前方にnearest_inf_rad_to_present_radが存在している場合、
        if 0.0 < math.cos(nearest_inf_rad_to_present_rad - position_meter[2]):
            # ロボットを回転させる（位置調整のため）
            self._lib["nav"].sendRotationTwist(nearest_inf_rad_to_present_rad, position_meter[2])
        """
