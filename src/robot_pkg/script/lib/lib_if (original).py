...
class LibIF:
    ...
    ## @param state = "judge"   被験者の進行方向選択を待っている時のGUI
    ...
    def changeGUI(
        ...
        if state == "start":
            self.pub_gui_state.publish(0)
        elif state == "move":
            self.pub_gui_state.publish(1)
        elif state == "wait":
            self.pub_gui_state.publish(2)
        elif state == "judge":
            self.pub_gui_state.publish(3)
        elif state == "error":
            self.pub_gui_state.publish(4)
