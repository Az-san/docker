...
class LibScan:
    ...
    def getAheadDistance(
        ...
        return min([min(self.sub_laserscan_data.ranges[0:5]),min(self.sub_laserscan_data.ranges[-5:-1])])


    #==================================================
    
    ## @fn getSideDistance
    ## @brief ロボット側方0.2(m)以内に障害物があるか調べ,
    ##障害物がある場合は30それから離れる角度を返す
    ## @param
    ## @return

    #==================================================
    def getSideProximity(
        self
    ):
        left = min(self.sub_laserscan_data.ranges[90-10:90+10])
        right = min(self.sub_laserscan_data.ranges[-90-10:-90+10])
        if right <= 0.4 and left >=0.4:
            return 0.523
        elif left <= 0.4 and right >= 0.4:
            return -0.523
        else:
            return 0
